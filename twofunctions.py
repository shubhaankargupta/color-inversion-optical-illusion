    ...
    for number in range(wbw//30):
        ...
        for n in range(step, w1, step):
          for p in range(step, h1 - step, step):
            draw.point((n, p))
        draw.ellipse((w1 / 2 - 5, h1 / 2 - 5, w1 / 2 + 5, h1 / 2 + 5), fill=(0, 0, 0))
        draw.line((x, y, x + offset, y), fill="white", width=widthofbar)image = Image.open(imgpath)
        inverted = ImageOps.invert(image)
        draw = ImageDraw.Draw(inverted)
        w1, h1 = inverted.size
...
imgpath = '/Users/guptashubhankar06/Desktop/red/hot-chili-pepper-powder-spice-as-background-natural-seasoning-texture_84485-560.jpeg'
